...
조건문 속에도 조건문이 들어갈 수 있다. 이미 하나의 조건으로 분기를 시켰는데, 그 속에서 또 분기를 시킬 필요가 있을 수 있다. 이 때 조건문 안에 또 다른 조건문을 넣는다. 그 예시는 아래와 같다.
...
if (y > 0) {
    ...
    if (x > 0) { cout << 4 << endl; }
    else if (x < 0) { cout << 3 << endl; }
}
```

위의 코드는 `(x, y)`에 해당하는 점이 위치하는 사분면을 출력하는 코드이다. 위처럼 조건문을 여러 번 사용하는 경우가 생길 수도 있다. 위는 `if` 안에 `if`가 들어갔지만, `if` 안에 `switch` 등과 같이 혼용하여 중첩을 시킬 수 있다. 또한, 2번이 아닌 3번, 4번, ... 무한히 중첩할 수 있다. PS에서는 과하게 조건문 안에 조건문을 사용하는 경우는 거의 없을 것이다.
